...
- **E-Mail-Bestätigung ist Pflicht** (`requireEmailVerification: true`). Eine Registrierung erzeugt erst nach dem Klick auf den Bestätigungslink eine Session; Bestätigungs- und Reset-Token laufen nach `ONE_HOUR_IN_SECONDS` ab (explizit gesetzt, nicht dem Better-Auth-Default überlassen). Versand über SMTP/Strato (`src/lib/server/email.ts`), Vorlagen in `src/lib/server/emailTemplates.ts` (reines Modul ohne `server-only`, damit testbar). **Better Auth verschluckt Fehler beim Mailversand** (`runInBackgroundOrAwait` awaitet und loggt nur) — eine Registrierung sieht auch dann erfolgreich aus, wenn nie eine Mail rausging. Deshalb loggt `sendEmail` selbst und deshalb gibt es `ResendVerification` auf Registrierungs-, Login- und Bestätigungsseite. Guards in `emailVerification.test.ts`.
- `/auth/verify-email` ist in `src/proxy.ts` von der „eingeloggt → weg von /auth"-Weiterleitung **ausgenommen**: `autoSignInAfterVerification` legt die Session an, bevor Better Auth dorthin weiterleitet.
