...
**Abnahme:** neuer User = Trial (42 Tage, Vollzugriff) ✅; nach Trial Free/Nur-Lese ✅; Create + GuV-Export serverseitig 403 `PRO_REQUIRED` ✅; Rechnungs-PDF/DSGVO-Export für Free frei ✅; Alt-User-Migration (aktive Abos → pro) ✅. **Operativ offen:** Stripe-Pro-Produkt (14,99 € brutto) anlegen + `STRIPE_PRICE_ID` setzen; End-to-End-Kauf im Stripe-Testmode verifizieren.
...
5. 🟡 **Stripe-Pro-Produkt angelegt (04.09.2026) — `STRIPE_PRICE_ID` in Vercel noch offen.** „Bilanz-Buddy Pro" existiert in Sandbox und Live mit monatlichem Preis **14,99 € brutto** (`tax_behavior: inclusive`, `tax_code: txcd_10103001`, ohne `trial_period_days` — die 42 Tage verwaltet `provisioning.ts`). Beide Preise tragen den Lookup-Key **`bb_pro_monthly`**; darüber findet man die `price_…`-ID im jeweiligen Modus, ohne sie hier abzulegen (das Repo ist öffentlich). Die Test-ID steht in `.env.local`, die Live-ID muss als `STRIPE_PRICE_ID` in Vercel gesetzt werden — ohne sie liefert `/api/stripe/checkout` 500 ([route.ts:18](../src/app/api/stripe/checkout/route.ts)). **Lokal fehlen weiterhin `STRIPE_SECRET_KEY` und `STRIPE_WEBHOOK_SECRET`** (beide leer in `.env.local`) — bis die Testschlüssel eingetragen sind, ist Billing lokal nicht durchspielbar.
6. ⬜ **Offen: Umsatzsteuer-Ausweis auf den Stripe-Rechnungen.** Der Checkout setzt kein `automatic_tax` und keine `default_tax_rates`; `tax_behavior: inclusive` allein sagt Stripe nur, dass 14,99 € die Steuer enthalten, weist sie aber auf der Rechnung nicht aus. Seit dem Verzicht auf die Kleinunternehmerregelung braucht eine Kleinbetragsrechnung nach § 33 UStDV mindestens den Steuersatz. Zwei Wege: Stripe Tax aktivieren (`automatic_tax: { enabled: true }`, 0,5 % je Transaktion, korrekt auch für EU-Ausland) oder eine feste Tax Rate 19 % inklusive anlegen und als `default_tax_rates` mitgeben (kostenlos, aber nur für Inlandskunden richtig).
7. ⬜ **Offen: Webhook `current_period_end`** hat einen +30-Tage-Fallback (Stripe-API-Version-Unsicherheit) → beim ersten echten Kauf gegen Stripe-Testmode verifizieren.
8. ⬜ **Offen: Supabase-Projekt abschalten** (Teil C in [`docs/DB-MIGRATION.md`](./DB-MIGRATION.md)) — die App braucht es nachweislich nicht mehr, es verursacht aber weiter Kosten.
9. ✅ **CI grün.** Next 16.3.0 schließt die High-Advisories (darunter Proxy-Bypass und Nonce-XSS, beide für diese App relevant). Das Gate blockiert jetzt auf `npm audit --omit=dev`; der vollständige Audit läuft informativ weiter und meldet zwei Advisories in der ESLint-Toolchain ohne Fix in der Range. Damit laufen Typecheck, Lint, Tests und Build in CI zum ersten Mal überhaupt durch.
10. ✅ **PR #10 → `master` gemergt** (`ad275e8`), Production deployt seitdem von `master`.
